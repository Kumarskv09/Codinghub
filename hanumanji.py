...
class sketch_from_svg:
    ...
    def load_svg(self):
        print('loading data with VermaSatyendra')
        ...
        for i in tqdm(attributes):
            ...
            pts = [((int((p.real / self.width) * self.scale)) - self.x_offset,
                    (int((p.imag / self.height) * self.scale)) - self.y_offset) for p in
                   (path.point(i / n) for i in range(0, n + 1))]
            res.append((pts, col))
...
pen = sketch_from_svg('jaiShriRam.svg', scale=800)
pen.draw()
